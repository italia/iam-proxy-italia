...
- **Container fails to start**: Check logs with `docker container logs iam-proxy-italia`. Ensure required directories and files exist (see [run-docker-compose.sh](README.run-docker-compose.sh.md)); copy from `env.example` to `.env` in `Docker-compose` and set variables.
