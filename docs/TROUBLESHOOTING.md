...
- **MongoDB connection refused**: If using the `mongo` or `demo` profile, ensure the MongoDB service is up (`docker compose ps`) and MongoDB env vars in `.env` match the compose configuration (see [docs/mongodb-env.md](mongodb-env.md)).
